...
export default function LandingPage() {
  ...
  return (
    <div className="marketing-theme min-h-screen bg-[var(--mkt-bg)] font-sans text-slate-900 antialiased selection:bg-teal-500/30 selection:text-teal-900">
      {/* Navbar */}
      <header className="sticky top-0 z-50 border-b border-[var(--mkt-border)] bg-white/80 backdrop-blur-xl">
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-10">
          <BrandLogo />
          <div className="hidden items-center gap-8 text-sm font-medium text-slate-600 md:flex">
            {navLinks.map((l) => (
              <Link key={l.href} href={l.href} className="transition-colors hover:text-teal-700">
                {l.label}
              </Link>
            ))}
          </div>
          <div className="hidden items-center gap-3 md:flex">
            <Link href="/login" className="px-4 text-sm font-medium text-slate-700 transition-colors hover:text-slate-900">
              Sign in
            </Link>
          ...
          <button type="button" className="text-slate-900 md:hidden" onClick={() => setMenuOpen(!menuOpen)} aria-label="Menu">
            {menuOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}
          </button>
        </nav>
        {menuOpen && (
          <div className="border-t border-slate-300/[0.06] bg-slate-50 px-6 py-4 md:hidden">
            {navLinks.map((l) => (
              <Link key={l.href} href={l.href} className="block py-2 text-slate-700" onClick={() => setMenuOpen(false)}>
                {l.label}
              </Link>
            ))}
            <div className="mt-4 flex flex-col gap-2">
              <Link href="/login" className="py-2 text-[#0f766e]">
                Sign in
              </Link>
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-2 lg:gap-16">
          <FadeIn>
            <h1 className="text-4xl font-bold leading-[1.1] tracking-tight sm:text-5xl lg:text-6xl">
              ...
              <span className="bg-gradient-to-r from-[#0f766e] to-emerald-300 bg-clip-text text-transparent">
                Digital Twin Tech
              </span>
            </h1>
            ...
            <div className="mt-10 flex items-center gap-4">
              ...
              <p className="text-sm text-slate-600">
                Trusted by <span className="font-semibold text-slate-900">1,200+</span> farmers across Kenya
              </p>
            </div>
          </FadeIn>

          <FadeIn delay={0.15} className="relative">
            <div className="absolute -inset-6 rounded-3xl bg-[#0f766e]/10 blur-3xl" />
            <div className="relative overflow-hidden rounded-2xl ring-1 ring-[#0f766e]/20 shadow-2xl shadow-black/60">
              <Image
      ...
      {/* Solutions */}
      <section id="solutions" className="border-t border-slate-300/[0.04] bg-white/50 px-6 py-20 lg:px-10">
          ...
          <div className="mt-14 grid gap-6 md:grid-cols-3">
            {[
              ...
              <FadeIn key={item.title} delay={i * 0.08}>
                <div className="glass-card group flex h-full flex-col p-8 transition-all hover:border-[#0f766e]/30 hover:shadow-[0_0_40px_rgba(0,230,118,0.08)]">
                  <div className="mb-6 flex h-14 w-14 items-center justify-center rounded-2xl border border-[#0f766e]/20 bg-[#0f766e]/10 text-[#0f766e] shadow-[0_0_24px_rgba(0,230,118,0.15)] transition-colors group-hover:bg-[#0f766e] group-hover:text-white">
                    <item.icon className="h-7 w-7" />
                  </div>
                  <h3 className="text-xl font-bold">{item.title}</h3>
                  <p className="mt-3 flex-1 text-sm leading-relaxed text-slate-600">{item.desc}</p>
                  <Link href="/register" className="mt-6 inline-flex items-center gap-1 text-sm font-medium text-[#0f766e] hover:gap-2 transition-all">
      ...
      <section id="about" className="px-6 py-20 lg:px-10">
        <div className="mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-2">
          <FadeIn>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#0f766e]">About AgriTwin</p>
            ...
            <div className="mt-10 grid grid-cols-3 gap-6">
              {[
                ...
                <div key={stat.label}>
                  <p className="text-2xl font-bold text-[#0f766e] sm:text-3xl">{stat.value}</p>
          ...
          <FadeIn delay={0.1} className="relative">
            <div className="glass-card overflow-hidden p-2 ring-1 ring-slate-200">
      ...
      {/* How it works */}
      <section id="how-it-works" className="border-t border-slate-300/[0.04] bg-white/50 px-6 py-20 lg:px-10">
        <div className="mx-auto max-w-7xl">
          <FadeIn className="text-center">
            <h2 className="text-3xl font-bold sm:text-4xl">How It Works</h2>
            <p className="mx-auto mt-4 max-w-xl text-slate-600">Four simple steps from signup to smarter harvests.</p>
          </FadeIn>
          <div className="relative mt-16 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            <div className="absolute top-10 left-[12%] right-[12%] hidden h-px border-t border-dashed border-[#0f766e]/20 lg:block" />
            {[
              ...
              <FadeIn key={item.title} delay={i * 0.06} className="relative flex flex-col items-center text-center">
                <div className="relative z-10 flex h-16 w-16 items-center justify-center rounded-full border-4 border-[#050a0e] bg-[#0f766e] text-xl font-bold text-white shadow-[0_0_30px_rgba(0,230,118,0.4)]">
                  {item.step}
                </div>
                <div className="mt-6 flex h-12 w-12 items-center justify-center rounded-xl border border-[#0f766e]/20 bg-[#0f766e]/10 text-[#0f766e]">
      ...
      <section id="use-cases" className="px-6 py-20 lg:px-10">
        <div className="mx-auto max-w-7xl">
          <FadeIn className="text-center">
            ...
            <p className="mx-auto mt-4 max-w-xl text-slate-600">From smallholders to agribusiness — scale at your pace.</p>
          </FadeIn>
          <div className="mt-14 grid gap-6 sm:grid-cols-2">
            {[
              { icon: Sprout, title: "Crop Farming", desc: "Maize, beans, tomatoes — monitor blocks and get localized advice.", color: "text-[#0f766e] border-[#0f766e]/25 bg-[#0f766e]/10" },
              ...
              <FadeIn key={item.title} delay={i * 0.06}>
                <div className="glass-card flex gap-5 p-6 transition-colors hover:border-slate-300/15">
      ...
      {/* Testimonials */}
      <section className="border-t border-slate-300/[0.04] bg-white/50 px-6 py-20 lg:px-10">
        <div className="mx-auto max-w-7xl">
          <FadeIn className="text-center">
            ...
            {[
              ...
              <FadeIn key={t.name} delay={i * 0.08}>
                <div className="glass-card flex h-full flex-col p-6">
                  <Quote className="h-8 w-8 text-[#0f766e]/60" />
                  <p className="mt-4 flex-1 text-sm leading-relaxed text-slate-700">&ldquo;{t.quote}&rdquo;</p>
                  <div className="mt-6 flex items-center gap-3 border-t border-slate-300/[0.06] pt-6">
                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#0f766e]/20 text-sm font-bold text-[#0f766e]">
                      {t.name.charAt(0)}
                    </div>
                    <div>
      ...
      <section className="px-6 py-16 lg:px-10">
        <FadeIn>
          <div className="relative mx-auto max-w-5xl overflow-hidden rounded-3xl border border-[#0f766e]/20 bg-gradient-to-br from-[#0f766e]/10 via-[#030608] to-[#050a0e] p-10 text-center shadow-[0_0_60px_rgba(0,230,118,0.12)] sm:p-14">
            <div className="absolute -right-20 -top-20 h-64 w-64 rounded-full bg-[#0f766e]/10 blur-3xl" />
      ...
      {/* Footer */}
      <footer id="contact" className="border-t border-slate-300/[0.06] bg-white px-6 pb-10 pt-16 lg:px-10">
        <div className="mx-auto max-w-7xl">
              ...
              <div className="mt-6 flex gap-4">
                {[Twitter, Linkedin, Instagram, Facebook].map((Icon, i) => (
                  <a
                    key={i}
                    href="#"
                    className="flex h-9 w-9 items-center justify-center rounded-lg border border-slate-300/10 text-slate-600 transition-colors hover:border-[#0f766e]/40 hover:text-[#0f766e]"
                    aria-label="Social"
                  >
            ...
            {[
              ...
              <div key={col.title}>
                ...
                <ul className="mt-4 space-y-2">
                  {col.links.map((link) => (
                    <li key={link}>
                      <Link href="#" className="text-sm text-slate-500 transition-colors hover:text-[#0f766e]">
                        {link}
                      </Link>
                    </li>
                  ))}
          ...
          <div className="mt-14 flex flex-col items-center justify-between gap-4 border-t border-slate-300/[0.06] pt-8 text-xs text-slate-500 sm:flex-row">
